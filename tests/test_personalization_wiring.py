...
def test_assistant_hybrid_search_wires_personalization(tmp_path):
    ...
    # data_dir 指向 tmp：不要让测试依赖（甚至改写）仓库里 OneDrive 托管的索引。
    # 否则全量跑时若 OneDrive 把 data/index/faiss_hnsw.index 脱水/锁定，
    # FAISS 写入会报 "No such file or directory" —— 单独跑却能过，属测试隔离缺陷。
